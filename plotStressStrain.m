...
%     try
%         SubSet = sArr(1:plotWhat.interval:end); % select tau of interest to plot
%     catch
%         % warning('Failed to limit tau to descrete intervals. Plotting all tau.')
%         SubSet = sArr;
%     end
...
%     hold on
%     for k = 1:length(SubSet)
%       hub = SubSet{k}(:,1:rdiv,1)
%       for b = 1:3
%         neg = find(hub(b,:,1)<0)
%         hub_norm(b,:) = hub(b,neg,1)/mat.stren{1}(2)
% 
%       rim = SubSet{k}(:,rdiv+1:2*rdiv,1)
% 
% 
%       plot(rArr*1000, circ_norm, [marker(k),'-'], 'MarkerIndices', 1:5:length(rArr), 'LineWidth', 1.5)
%       plot(rArr*1000, ax_norm, [marker(k),'--'], 'MarkerIndices', 1:5:length(rArr), 'LineWidth', 1.5)
%       plot(rArr*1000, rad_norm, [marker(k),'-.'], 'MarkerIndices', 1:5:length(rArr), 'LineWidth', 1.5)
%     end

%     grid on
%     xlabel(['Radius [', lng_unit, ']'])
%     ylabel(['Max Str [\omega_i/\omega]'])
%     legend(legTxt, 'Location', 'northeast')
% 
%     figure(), hold on
